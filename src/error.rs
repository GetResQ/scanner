...
#[derive(Debug, Error)]
pub enum CheckError {
    /// The check failed but produced no annotations.
    #[error("check failed with exit code {exit_code:?} but produced no annotations")]
    NoAnnotations { exit_code: Option<i32> },
...
mod tests {
    ...
    fn check_error_display() {
        let err = CheckError::NoAnnotations { exit_code: Some(1) };
        assert!(err.to_string().contains("exit code"));
        assert!(err.to_string().contains("no annotations"));
    }
