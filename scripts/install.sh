...
RELEASE_TAG=${SCANNER_RELEASE_TAG:-latest}
TARGET_ASSET=${SCANNER_ASSET_NAME:-}
...
if [[ -n "$TARGET_ASSET" ]]; then
  ...
  ASSET_NAME="scanner-${RELEASE_TAG}-${TARGET_TRIPLE}.tar.gz"
fi
...
if [[ -z "$BINARY_PATH" ]]; then
  echo "error: could not locate the scanner binary in downloaded asset" >&2
  exit 1
fi

mkdir -p "$INSTALL_DIR"
install -m 755 "$BINARY_PATH" "$INSTALL_DIR/scanner"

echo "Installed scanner $RELEASE_TAG to $INSTALL_DIR/scanner"

if [[ ":$PATH:" != *":$INSTALL_DIR:"* ]]; then
  cat <<MSG
To run scanner from anywhere, add the install directory to your PATH (e.g. ~/.zshrc or ~/.bashrc):
